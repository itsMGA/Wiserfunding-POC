...
class Partner_Report_User_Facade(BaseFacadeClass):
    response = None
    ...
    def executeSuccessfulRequest(self, environmentConfigs, testData):
        url = environmentConfigs.partner_ReportUser_URL
        url = f"{url}/{self.getUser_id(environmentConfigs, testData)}"
        headers = {
