...
class Partner_Report_Device_Facade(BaseFacadeClass):
    response = None
    ...
    def executeSuccessfulRequest(self, environmentConfigs, testData):
        url = environmentConfigs.partner_ReportDevice_URL
        url = f"{url}/{self.getDevice_id(environmentConfigs, testData)}"
        ...
        # print(self.response.json(), "THIS TE RESP")
        return self
